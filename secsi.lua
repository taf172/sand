...
function secsi.update(dt, group)
  ...
  for k, sys in ipairs(systems) do
    if sys.group == group then
      ...
      -- For each entity in small list
        -- get components
        -- update
        -- if components different then resort for relevant filters only??
      -- end
      for i, e in pairs(entities) do
        ---[[
        --if check(e, sys.filter) then
          sys.update(e, dt)
        --end
        --]]
      end
